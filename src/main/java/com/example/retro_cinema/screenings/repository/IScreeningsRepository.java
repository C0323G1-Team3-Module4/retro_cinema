...
public interface IScreeningsRepository extends JpaRepository<Screenings, Integer> {
    @Query(value = "select s.id, s.date_movie as dateMovie, m.movie_name as movieName, m.price, st.start_time as startTime,s.showtime_id as showtimeId, st.end_time as endTime, r.room_name as roomName\n" +
            "from screenings s\n" +
    ...
    List<IScreeningsDto> getAllScreenings();
}
